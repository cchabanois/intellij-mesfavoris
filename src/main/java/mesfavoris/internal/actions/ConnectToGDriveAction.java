...
public class ConnectToGDriveAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent event) {
        if (event.getProject() == null) {
            Messages.showErrorDialog("No project is open", "Connection Error");
            return;
        }
        ...
        ProgressManager.getInstance().run(new Task.Modal(event.getProject(), "Connecting to Google Drive", true) {
            ...
            public void run(@NotNull ProgressIndicator indicator) {
                try {
                    connectAndDisplayTokens(event.getProject(), indicator);
                } catch (Exception e) {
    ...
    private void connectAndDisplayTokens(com.intellij.openapi.project.Project project, ProgressIndicator indicator) throws Exception {
        ...
        try {
            ...
            // Read the stored credentials from PasswordSafe
            PasswordSafeDataStoreFactory dataStoreFactory = new PasswordSafeDataStoreFactory();
