...
public class GDriveConnectionManager {
	...
	public GDriveConnectionManager(File dataStoreDir, String applicationName, String applicationFolderName) {
		this(dataStoreDir, new AuthorizationCodeIntellijApp.Provider(), applicationName, applicationFolderName);
	}
	...
	public void connect(ProgressIndicator monitor) throws IOException {
		...
		try {
			...
			UserInfo authenticatedUser = getAuthenticatedUser(drive, monitor);
			if (monitor != null) {
				monitor.setFraction(0.95);
			}
			saveUser(authenticatedUser, monitor);
			synchronized (this) {
				this.drive = drive;
	...
	private Credential authorize(final ProgressIndicator monitor) throws IOException {
		if (monitor != null) {
			monitor.setText("Authorizes the application to access user's protected data on Google Drive");
		}
		// load client secrets
		...
		CancellableLocalServerReceiver cancellableReceiver = new CancellableLocalServerReceiver(
				localServerReceiver, monitor);
		AuthorizationCodeInstalledApp authorizationCodeInstalledApp = authorizationCodeInstalledAppProvider.get(flow,
				cancellableReceiver, monitor);
		return authorizationCodeInstalledApp.authorize("user");
	}

	private UserInfo getAuthenticatedUser(Drive drive, ProgressIndicator monitor) throws IOException {
		if (monitor != null) {
			monitor.setText("Getting authenticated user info");
		}
	...
	public void disconnect(ProgressIndicator monitor) {
	...
	private UserInfo loadUserInfo() {
		...
		try {
			return persister.loadUser(new EmptyProgressIndicator());
		} catch (IOException e) {
			LOG.warn("Could not load gdrive user info", e);
			return null;
		}
	}

	private void saveUser(UserInfo user, ProgressIndicator monitor) {
		UserInfoPersister persister = new UserInfoPersister(new File(dataStoreDir, USER_FILENAME));
		try {
			persister.saveUser(user, monitor);
		} catch (IOException e) {
			LOG.warn("Could not save gdrive user info", e);
		}
	}
